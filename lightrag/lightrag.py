...
class LightRAG:
    ...
    chunk_token_size: int = field(default=int(os.getenv("CHUNK_SIZE", 800)))
    """Maximum number of tokens per text chunk when splitting documents."""

    chunk_overlap_token_size: int = field(
        default=int(os.getenv("CHUNK_OVERLAP_SIZE", 50))
    )
    ...
    def _run_async_safely(self, async_func, action_name=""):
        ...
        try:
            ...
            if loop.is_running():
                ...
                self.BACKGROUNG_TASKS.add(task)
                task.add_done_callback(
                    lambda t: (
                        logger.info(f"{action_name} completed!"),
                        self.BACKGROUNG_TASKS.discard(task)
                    )
                )
            else:
